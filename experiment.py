...
def main():
    ...
    #Insert you image filenames here
    ...
    assert 100 < img1.shape[0] and img1.shape[0] <= 512
    assert 100 < img1.shape[1] and img1.shape[1] <= 512    
    assert 100 < img2.shape[0] and img2.shape[0] <= 512
    assert 100 < img2.shape[1] and img2.shape[1] <= 512  
    ...
    copyPasteMiddle(mono1, mono2, (100,100))
          
    cv2.imwrite('output/ps1-3-a-1.png', mono2)
    ...
    ##4c
    shift_green = shiftImageLeft(img1_green, 30)
    ...
    noisy_green = addNoise(img1_f64, 1, sigma)
    cv2.imwrite('output/ps1-5-a-1.png', np.clip(noisy_green,0,255))

    ##5b
    noisy_blue = addNoise(img1, 0, sigma)
